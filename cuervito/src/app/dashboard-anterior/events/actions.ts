...
import { busUserDashboardCache, purgarEvento, slugify, uniqueSlug } from "~/app/dashboard/_nucleo";
...
export async function createEventAction(
  ...
  revalidatePath("/dashboard-anterior/events");
...
export async function updateEventAction(
  ...
  busUserDashboardCache(session.user.id);
  revalidatePath("/dashboard-anterior/events");
  revalidatePath(`/dashboard-anterior/events/${id}`);
  redirect(`/dashboard-anterior/events/${id}`);
}
...
export async function archiveEventAction(id: string): Promise<void> {
  ...
  if (!ev || ev.ownerId !== session.user.id) redirect("/dashboard-anterior/events");

  await db.event.update({ where: { id }, data: { status: "ARCHIVED", isPublished: false } });
  busUserDashboardCache(session.user.id);
  revalidatePath("/dashboard-anterior/events");
  redirect("/dashboard-anterior/events");
}
...
export async function deleteEventAction(id: string): Promise<void> {
  ...
  const ok = await purgarEvento(id, session.user.id);
  if (!ok) redirect("/dashboard-anterior/events");
