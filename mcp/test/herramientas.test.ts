...
describe("estado de salud", () => {
  ...
  it("fotos esperando y nada terminado en media hora: down, con alerta crítica", async () => {
    cliente = await clienteConectado(conFotos({ pendientes: 40, trabajables: 40, edad_ultimo_exito_s: 3_600 }));
    const r = await llamar(cliente, "get_health");
  ...
  it("sólo fotos apartadas y la cola ociosa: degraded, no down", async () => {
    ...
    cliente = await clienteConectado(
      conFotos({ pendientes: 10, pendientes_1h: 10, apartadas: 10, trabajables: 0, trabajables_1h: 0, edad_ultimo_exito_s: 250_000 }),
    );
    const r = await llamar(cliente, "get_health");
