...
| Estado | Cuándo |
|---|---|
| `down` | Hay fotos libres hace más de 15 minutos, o una unidad colgada, y la cola no hizo nada en 30 minutos (`processing_stalled`). O hay 10 o más fotos que fallaron en las últimas 2 horas y ninguna terminó en 30 minutos (`processing_failing`). |
...
El atasco no se mide con "hace cuánto que no termina nada": en una semana sin carreras eso son días, y la primera foto que se libera haría decir `down` hasta que la cola se despierte (duerme hasta 10 minutos cuando no tiene trabajo). Por eso cuenta sólo lo que la cola ya tendría que haber tomado, y lo compara con el último éxito, de vista previa o de reconocimiento. Los reclamos no cuentan como actividad: en una falla como la del 16 al 20/9 la cola reclama sin parar y no termina nada. Las fotos *apartadas* no cuentan para `down`: la cola dejó de intentarlas a propósito. Para `processing_failing` cuentan las que fallaron en las últimas 2 horas (la base no guarda cuándo, pero se deduce del lease menos la espera que le tocaba): una que espera un reintento lento de hace más puede tener la causa ya arreglada, y eso es `photos_retrying`, no una caída. `timings` sale de las últimas 20 fotos procesadas en las últimas 6 horas; con menos de 3, es `null` y la razón va en `unavailable_reason`.
